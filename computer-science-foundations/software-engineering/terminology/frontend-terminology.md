...
- **[Feature → Boundary → Contract → Schema → Type → Adapter](feature-boundary-contract-schema-type-adapter.md)**: A reasoning sequence connecting ownership, responsibility changes, expected representations, optional runtime verification, optional compile-time knowledge, and optional translation between contracts.
- **[Boundary Contract](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#a-contract-is-not-just-a-type)**: A representation's required shape and meaning at one point where data enters, leaves, or changes responsibility.
- **[Form Contract](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#form-contract-and-field-names)**: The browser-friendly value shape, field names, defaults, normalization, and client validation owned by a form boundary.
- **[API Request Contract](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#api-request-contracts)**: The parameters or body representation a frontend is permitted to send to an endpoint.
- **[API Response Contract](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#api-response-contracts-and-network-trust)**: The expected runtime shape and meaning of data returned by an API before it enters application state or a cache.
- **[Executable Contract / Runtime Schema](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#a-schema-is-an-executable-contract)**: Runtime code that can parse or reject actual values while also serving as the source of an inferred compile-time type.
- **[Schema-First Contract](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#a-schema-is-an-executable-contract)**: Defining the executable runtime schema first and deriving its TypeScript input or output type instead of maintaining a separate handwritten type.
- **[Semantic Adapter](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#adapters-translate-meaning-between-contracts)**: A function or layer that translates one boundary contract into another while preserving workflow meaning, such as converting an empty form value into an omitted creation field.
- **[Feature-Oriented Structure](../../../frontend-development/data-across-boundaries/contracts-boundaries-schemas-and-adapters.md#feature-oriented-structure)**: Organizing application code by business capability first, then separating responsibilities such as UI, data access, contracts, and state inside each feature.
...
- **[Component-Identity Regression](../../../framework-tooling/frontend/react/rendering-model.md#component-identity-regressions)**: A defect where a component loses its stable identity across renders, causing the reconciler to unmount the previous instance, destroy DOM nodes, and wipe local state.
