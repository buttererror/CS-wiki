# Computer Science Foundations

## Purpose

This directory contains foundational distinctions used to classify and connect software concepts throughout the knowledge base.

## Document Index

- [Software Taxonomy](software-taxonomy.md) — distinguishes paradigms, patterns, architectural concepts, tools, and implementations.
- [Software Architecture](software-architecture.md) — distinguishes software systems, system architecture, architectural styles, and architectural patterns.

## Related Concepts

- [Programming Paradigms](../programming-paradigms/README.md)
- [Software Design Principles](../software-design-principles/README.md)
- [Software Engineering Terminology](../terminology/README.md)
- [Design Patterns](../design-patterns/README.md)
- [Communication Patterns](../communication-patterns/README.md)
- [Architectural Styles](../architectural-styles/README.md)
- [Architectural Patterns](../architectural-patterns/README.md)
- [Frameworks, Libraries, and Tooling](../framework-tooling/README.md)
